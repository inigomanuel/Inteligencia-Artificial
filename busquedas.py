...
class Ventana(tk.Frame):
    ...
    def create_widgets(self):
        v = tk.IntVar()
        menu_msg = "" \
                   "\nTarea 01: Algoritmo Búsqueda a Ciegas y Búsqueda Heurística"
        self.menu_tktext = Label(root, text=menu_msg)
        self.menu_tktext.pack()

        self.boton1 = Button(root, text="Búsqueda ciega", command=lambda: Main(0,entry.get(),entry2.get(),entry3.get(),entry4.get(),entry5.get()))
        self.boton2 = Button(root, text="Búsqueda heurística - Hill Climbing", command=lambda: Main(2,entry.get(),entry2.get(),entry3.get(),entry4.get(),entry5.get()))
        self.boton1.place(x=90, y=250)
        self.boton2.place(x=300, y=250)
...
class Graph():
    ...
    def hacer_conexiones(self):
        for k in list(self.G.nodes()):
            ...
            if(neigh[1][1] != -1):
                self.G.add_edge(k,neigh[1][1],color='gray')

            if(neigh[1][2] != -1):
                self.G.add_edge(k,neigh[1][2],color='gray')

            if(neigh[2][1] != -1):
                self.G.add_edge(k,neigh[2][1],color='gray')

            if(neigh[1][2] != -1 and neigh[2][1] != -1):
                self.G.add_edge(neigh[1][2],neigh[2][1],color='gray')
...
###################################################################################################################################################
#Algoritmo Ciego
    def Ciego(self,StartX, StartY, EndX, EndY):
        INICIO=0
        FIN=0
    ...
    if(valor==0):
        ...
        for i in nd_deleted:
            if nd_deleted[i][0]==ax and nd_deleted[i][1]==ay:
                window = Tk()
                window.title("Error")
                lbl=Label(window,text="El nodo final fue eliminado por favor escoja otro :( ")
        ...
        for i in nd_deleted:
            if nd_deleted[i][0]==int(sx) and nd_deleted[i][1]==int(sy):
                ...
                break
        if se_ejecuta:
            grafo.Ciego(int(sx),int(sy),ax,ay)
            grafo.print_grafo()
    elif(valor==2):
        grafo=Graph(int(n),int(n))
        ax=int(x)
...
if __name__ == '__main__':
    ...
    entry=tk.Entry(root)
    entry.insert(0,"'x' FIN'")
    entry.place(x=320,y=150)
    entry2=tk.Entry(root)
    entry2.insert(0,"'y' FIN'")
    entry2.place(x=320,y=175)
    
    entry3=tk.Entry(root)
    entry3.insert(0,"'x' INICIO'")
    entry3.place(x=80,y=150)
    entry4=tk.Entry(root)
    entry4.insert(0,"'y' INICIO'")
    entry4.place(x=80,y=175)

    entry5=tk.Entry(root)
    entry5.insert(0,"Ingrese 'n' Espacios")
    entry5.place(x=210,y=65)
